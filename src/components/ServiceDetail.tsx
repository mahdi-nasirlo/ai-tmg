...
export default function ServiceDetail() {
  const [activeTab, setActiveTab] = useState<'overview' | 'docs' | 'examples' | 'api'>('overview');
  ...
  const serviceData = {
    ...
    description: 'سیستم جامع مدیریت و اتوماسیون فرآیندهای سازمانی با قابلیت طراحی گردش کار پیشرفته، مدیریت وظایف، نوتیفیکیشن خودکار و گزارش‌گیری دقیق',
    features: [
      {
          ...
          <div className="flex items-start justify-between flex-wrap gap-4">
            <div>
              <div className="inline-flex items-center gap-2 bg-blue-500/20 border border-blue-400/30 px-3 py-1 rounded-full mb-3">
                <span className="text-blue-300 text-sm">{serviceData.category}</span>
              </div>
              <h1 className="text-4xl md:text-5xl font-bold text-white mb-3 neon-text">
                {serviceData.title}
              </h1>
              <p className="text-white/80 text-lg max-w-3xl">{serviceData.description}</p>
            </div>

            <div className="flex flex-col gap-2 text-right">
              <div className="text-white/60 text-sm">نسخه: <span className="text-white font-semibold">{serviceData.version}</span></div>
              <div className="text-white/60 text-sm">آخرین بروزرسانی: <span className="text-white">{serviceData.lastUpdate}</span></div>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="space-y-8 animate-fade-in">
              ...
              <div className="bg-gradient-to-br from-white/5 to-white/10 backdrop-blur-sm rounded-xl p-8 border border-white/20">
                ...
                <div className="bg-black/40 rounded-lg p-4 font-mono text-sm border border-white/10">
                  <pre className="text-green-400 whitespace-pre-wrap">{serviceData.installation}</pre>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-white/5 to-white/10 backdrop-blur-sm rounded-xl p-8 border border-white/20">
                ...
                <div className="bg-black/40 rounded-lg p-4 font-mono text-sm border border-white/10 relative group">
                  ...
                  <pre className="text-blue-300 whitespace-pre-wrap pr-12">{serviceData.quickStart}</pre>
                </div>
              </div>
            </div>
          )}
              ...
              {serviceData.examples.map((example, index) => (
                ...
                >
                  ...
                  <div className="bg-black/40 rounded-lg p-4 font-mono text-sm border border-white/10 relative">
                    ...
                    <pre className="text-purple-300 whitespace-pre-wrap pr-12">{example.code}</pre>
                  </div>
                </div>
              ))}
            </div>
              ...
              {serviceData.apiEndpoints.map((endpoint, index) => (
                ...
                >
                  <div className="flex flex-col md:flex-row md:items-center gap-4 mb-3">
                    <span className={`inline-flex px-3 py-1 rounded-lg font-mono text-sm font-bold ${
                  ...
                  <div className="text-sm text-white/50">
                    پارامترها: <code className="text-cyan-400">{endpoint.params}</code>
                  </div>
